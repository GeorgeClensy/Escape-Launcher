...
class OnboardingRepositoryImpl @Inject constructor(
    ...
    override val firstTime: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.FIRST_TIME] ?: true }
    override suspend fun setFirstTime(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.FIRST_TIME] = enabled }
    }
    override val firstTimeHelp: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.FIRST_TIME_HELP] ?: true }
    override suspend fun setFirstTimeHelp(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.FIRST_TIME_HELP] = enabled }
    }
    override val isOnDefaultLauncherOnboarding: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.IS_ON_DEAFAULT_LAUNCHER_ONBOARDING_PAGE] ?: false }
