...
class ClockRepositoryImpl @Inject constructor(
    ...
    override val twelveHourClock: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.TWELVE_HOUR_CLOCK] ?: DefaultSettings.TWELVE_HOUR_CLOCK }
    override suspend fun setTwelveHourClock(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.TWELVE_HOUR_CLOCK] = enabled }
    }
    override val showClock: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.SHOW_CLOCK] ?: DefaultSettings.SHOW_CLOCK }
    override suspend fun setShowClock(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.SHOW_CLOCK] = enabled }
    }
    override val bigClock: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.BIG_CLOCK] ?: DefaultSettings.BIG_CLOCK }
    override suspend fun setBigClock(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.BIG_CLOCK] = enabled }
    }
    override val showDate: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.SHOW_DATE] ?: DefaultSettings.SHOW_DATE }
