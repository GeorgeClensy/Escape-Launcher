...
class WeatherSettingsRepositoryImpl @Inject constructor(
    ...
    override val showWeather: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.SHOW_WEATHER] ?: false }
    override suspend fun setShowWeather(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.SHOW_WEATHER] = enabled }
    }
    override val useFahrenheit: Flow<Boolean> = dataStore.data.map { it[PreferencesKeys.USE_FAHRENHEIT] ?: false }
    override suspend fun setUseFahrenheit(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.USE_FAHRENHEIT] = enabled }
    }
    override val weatherAppPackage: Flow<String> = dataStore.data.map { it[PreferencesKeys.WEATHER_APP_PACKAGE] ?: "" }
